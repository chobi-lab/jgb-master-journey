...
+ [ ] 入札のテールなどの情報はBBGからとれる？30yr JGB auction tail
  + [ ] 財務省のHPから過去分のデータが取得可能。[過去の入札結果] (https://www.mof.go.jp/jgbs/reference/appendix/jgb_historical_data.xls)
    + [ ] 財務省のHPから過去の入札結果のデータを抽出し可視化する。
    + [ ] また応札倍率・テールを計算し分析に生かす。応札倍率の定義はオファー額に対する応札額の比率であり、テールは平均落札価格と最低落札価格の差である。応札倍率が低ければ、需要が低い。テールは短ければ、堅調な需要があったことを示し、長ければ、低い価格で落札できた投資家が存在。
    + [ ] また、入札前の先回り買い等のアノマリーを検証する。
